...
import { addTest } from '../../../../actions/test.actions';

export default function TestFormDialog({ open, onClose }) {
  ...
  const handleSubmit = () => {
    ...
    dispatch(addTest(formData, config)).then(() => {
      alert('Test ajoutée avec succès! ✅');
      onClose();
      setFormValues(initialFormValues);
    }).catch((error) => {
      console.error('Erreur lors de l\'ajout de la test:', error);
      alert('Erreur lors de l\'ajout de la test ❌');
    });
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogContent>
        <Typography variant="h6" gutterBottom>Ajouter une Test</Typography>
